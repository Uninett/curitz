...
class listbox():
    ...
    """
    def __init__(self, nlines, ncols, begin_y=0, begin_x=0, current_selected_arrow="", lr_border=True):
          ...
          for i in range(page_start, self.pagesize + page_start):
              ...
              if len(self.elements) == 0:
                  ...
                  self.box.addstr(i + 1 - page_start,
                                  start_at,
                                  "{}{}".format(ar,(curr_element.text)[0:self.size.length - 2].ljust(self.size.length-2)),
                                  *c)
